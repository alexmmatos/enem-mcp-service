...
`ExamService.createExam` (ver [api-reference.md](./api-reference.md#create_exam)) busca as questões já semeadas via `fetchEnemQuestions` (`enem.repository.ts`), mapeia cada uma para o formato de `questions` com `enemQuestionToData` (`question.service.ts`) e grava (upsert) nessa coleção antes de criar a tentativa. Isso normaliza a prova — chamadas repetidas para o mesmo ano/índice não duplicam documentos — e permite que todo o resto do sistema (`submit_answer`, relatório, exposição de dados) trate as questões da mesma forma, sem nenhum código especial a partir daí.

Modelagem:
- `_id` da questão: `enem-{year}-{index}`, onde `index` é o número oficial da questão (1–185).
...
- Sem shuffle: a ordem persistida em `questionIds` é a ordem oficial da prova, a partir do início (offset 0).
