...
- [data-model.md](./data-model.md) — schema Prisma e cache local das questões do ENEM.

## Visão geral em uma frase

Servidor MCP (Skybridge + Prisma) que cria e conduz provas do ENEM com questões oficiais de `api.enem.dev`: o LLM traduz intenção em ano e quantidade, mas toda questão, resposta, pontuação e progresso vive no banco — nunca no histórico da conversa.
