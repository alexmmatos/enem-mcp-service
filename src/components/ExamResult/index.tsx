...
export function ExamResult({ report, onRestart }: { report: ExamReport; onRestart: () => void }) {
  return <section className={styles.result}>
    ...
    {report.reviewQuestions.length ? <><h3>Revisão</h3><ol className={styles.wrongList}>{report.reviewQuestions.map((item) => <li key={item.questionId} className={item.correct ? styles.correct : styles.incorrect}><strong>{item.statement}</strong><br /><span className={styles.muted}>{item.explanation}</span></li>)}</ol></> : null}
